...
def compute_speed_for_duration(
    actual_s: float, target_s: float,
    min_speed: float = 0.5, max_speed: float = 4.0,
) -> tuple[float, bool, bool]:
    """Compute speed factor to fit audio into target duration.

    Returns:
        (speed_factor, needs_trim, needs_pad)
        - speed_factor: clamped to [min_speed, max_speed]
        - needs_trim: True if audio still too long after max speed
        - needs_pad: True if audio still too short after min speed
    """
    if actual_s <= 0 or target_s <= 0:
        return 1.0, False, False

    raw_speed = actual_s / target_s
    clamped = max(min_speed, min(max_speed, raw_speed))
    needs_trim = raw_speed > max_speed
    needs_pad = raw_speed < min_speed
    return clamped, needs_trim, needs_pad


def trim_audio(audio: np.ndarray, target_samples: int, fade_ms: int = 50, sr: int = 24000) -> np.ndarray:
    """Trim audio to target length with a fade-out at the cut point."""
    if len(audio) <= target_samples:
        return audio
    trimmed = audio[:target_samples].copy()
    fade_samples = min(int(sr * fade_ms / 1000), target_samples)
    if fade_samples > 0:
...
def pad_audio(audio: np.ndarray, target_samples: int) -> np.ndarray:
    """Pad audio with silence to reach target length."""
    if len(audio) >= target_samples:
        return audio
